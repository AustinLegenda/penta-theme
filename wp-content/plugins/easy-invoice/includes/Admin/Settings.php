...
use MatrixAddons\EasyInvoice\Admin\Settings\BusinessSettings;
use MatrixAddons\EasyInvoice\Admin\Settings\EmailsSettings;
use MatrixAddons\EasyInvoice\Admin\Settings\GeneralSettings;
use MatrixAddons\EasyInvoice\Admin\Settings\PaymentSettings;
use MatrixAddons\EasyInvoice\Admin\Settings\InvoiceSettings;
use MatrixAddons\EasyInvoice\Admin\Settings\QuoteSettings;use MatrixAddons\EasyInvoice\Admin\Settings\TaxSettings;
use MatrixAddons\EasyInvoice\Admin\Settings\TextsSettings;
...
class Settings {
		...
		public static function get_settings_pages() {
			...
			if ( empty( self::$settings ) ) {
				...
				$settings[] = new EmailsSettings();
				$settings[] = new TextsSettings();


				self::$settings = apply_filters( 'easy_invoice_get_settings_pages', $settings );
			}
		...
		public static function save() {
			...
			check_admin_referer( 'easy-invoice-settings' );

			// Trigger actions.
			do_action( 'easy_invoice_settings_save_' . $current_tab );
			do_action( 'easy_invoice_update_options_' . $current_tab );
			do_action( 'easy_invoice_update_options' );

			self::add_message( __( 'Your settings have been saved.', 'easy-invoice' ) );

			// Clear any unwanted data and flush rules.
			update_option( 'easy_invoice_queue_flush_rewrite_rules', 'yes' );

			do_action( 'easy_invoice_settings_saved' );
		}
		 ...
		 * Settings page.
		 *
		 * Handles the display of the main easy-invoice settings page in admin.
		 */
		public static function output() {
			global $current_section, $current_tab;

			do_action( 'easy_invoice_settings_start' );


			// Get tabs for the settings page.
			$tabs = apply_filters( 'easy_invoice_settings_tabs_array', array() );
		 ...
		 * Output admin fields.
		 *
		 * Loops though the easy-invoice options array and outputs each field.
		 *
		...
		public static function output_fields( $options ) {
			foreach ( $options as $value ) {
				...
				$hidden_class = $value['type'] === 'hidden' ? 'easy-invoice-hide': '';
				...
				switch ( $value['type'] ) {
					...
					case 'title':
						...
						if ( ! empty( $value['id'] ) ) {
							do_action( 'easy_invoice_settings_' . sanitize_title( $value['id'] ) );
						}
						break;
					...
					case 'sectionend':
						if ( ! empty( $value['id'] ) ) {
							do_action( 'easy_invoice_settings_' . sanitize_title( $value['id'] ) . '_end' );
						}
						echo '</table>';
						if ( ! empty( $value['id'] ) ) {
							do_action( 'easy_invoice_settings_' . sanitize_title( $value['id'] ) . '_after' );
						}
						break;
					...
					case 'image':
						...
						?><tr valign="top" class="<?php echo esc_attr($hidden_class) ?>">
							...
							<td class="easy-invoice-image-field forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
								...
								<input
									...
									style="<?php echo esc_attr( $value['css'] ); ?>"
									value="<?php echo esc_attr( $option_value ); ?>"
									class="easy-invoice-image-value-hidden <?php echo esc_attr( $value['class'] ); ?>"
									placeholder="<?php echo esc_attr( $value['placeholder'] ); ?>"
									<?php echo esc_attr(implode( ' ', $custom_attributes )); ?>
									/><?php echo esc_html( $value['suffix'] ); ?> <?php echo ($description); ?>


									<div class="easy-invoice-image-field-wrap">
                                        <a class="easy-invoice-image-field-add <?php echo $option_value > 1 ? 'easy-invoice-hide' : ''; ?>" href="#"
                                           data-uploader-title="Add new image"
                                           data-uploader-button-text="Add new image">
                                            <img src="<?php echo esc_url(EASY_INVOICE_ASSETS_URI) ?>images/upload-image.png">
                                            <h3>Click here to browse file</h3>
                                            <p>Supports: JPG, JPEG, PNG</p>
                                        </a>
                                        <div class="image-container<?php echo $option_value < 1 ? ' easy-invoice-hide' : ''; ?>">
                                            <?php
                                                ...
                                                <div class="image-wrapper" data-url="<?php echo esc_url_raw($image_src) ?>">
                                                    <div class="image-content"><img
                                                        ...
                                                        <div class="image-overlay"><a
                                                                    class="easy-invoice-image-delete easy-invoice-remove-image dashicons dashicons-trash"></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            <?php } ?>
						...
						<tr valign="top">
							...
							<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">&lrm;
								<input
									...
									style="<?php echo esc_attr( $value['css'] ); ?>"
									value="<?php echo esc_attr( $option_value ); ?>"
									class="<?php echo esc_attr( $value['class'] ); ?> easy-invoice-colorpicker easy-invoice-hide"
									placeholder="<?php echo esc_attr( $value['placeholder'] ); ?>"
									<?php echo esc_attr(implode( ' ', $custom_attributes )); ?>
									/>&lrm; <?php echo ($description); ?>
									<div class="wp-picker-container easy-invoice-color-picker-container">
                                        <button type="button" class="button wp-color-result easy-invoice-color-picker-button" aria-expanded="false" style="background-color:<?php echo esc_attr($option_value) ?>;">
					...
					case 'textarea':
						...
						<tr valign="top">
                            ...
                            $editor = isset($value['editor'])  &&  (boolean)$value['editor'];
                    if ($editor) {
                                $editor_settings = $value['editor_settings'] ?? array();
					...
					case 'multicheckbox':
						$option_value     = self::get_option( $value['id'], $value['default'] );
						$display_type     = $value['display_type'] ?? 'inline';
						$visibility_class = array();
						...
						if ( 'option' === $value['show_if_checked'] ) {
							$visibility_class[] = 'show_options_if_checked';
 							...
 							?>
								<tr valign="top" class="<?php echo esc_attr( implode( ' ', $visibility_class ) ); ?>">
									<th scope="row" class="titledesc"><?php echo esc_html( $value['title'] ); ?></th>
									<td class="forminp forminp-multi-checkbox easy-invoice-multicheckbox <?php echo esc_attr($display_type) ?>">
					...
					case 'single_select_page':
						...
						<tr valign="top" class="single_select_page">
							...
							<td class="forminp">
								<?php echo str_replace( ' id=', " data-placeholder='" . esc_attr__( 'Select a page&hellip;', 'easy-invoice' ) . "' style='" . $value['css'] . "' class='" . $value['class'] . "' id=", wp_dropdown_pages( $args ) ); ?> <?php echo ($description); ?>
							</td>
						</tr>
						<?php
						break;
						...
						<tr valign="top" class="single_select_page">
							...
							<td class="forminp">
								<div class="easy-invoice-setting-tab-options">
								...
								foreach($repeator_value as $rep_key=> $rep_val){
									...
									?>
									<input type="text" class="easy_invoice_frontend_tabs_available_options_label" name="<?php echo esc_attr( $value['id'] ); ?>[<?php echo esc_attr($rep_key); ?>][label]" value="<?php echo esc_attr($rep_val['label']) ?>"/>
									<input class="easy_invoice_frontend_tabs_available_options_icon icopick" type="text" name="<?php echo esc_attr( $value['id'] ); ?>[<?php echo esc_attr($rep_key); ?>][icon]" value="<?php echo esc_attr($rep_val['icon']) ?>"/>
                                    <label class="easy-invoice-switch-control">
								...
								?>
									<button type="button" class="button" id="easy-invoice-setting-tab-option-add-new-tab"><?php echo __( 'Add New', 'easy-invoice' ) ?></button>
								</div>
							</td>
						</tr>
						<?php
						...
						case "content":
							...
							$allowed_html=$value['allowed_html'] ?? array();
						...
						?><tr valign="top" class="<?php echo esc_attr($hidden_class) ?>">
							...
							<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
								<div class="<?php echo esc_attr($value['class']) ?>"><?php echo wp_kses($content, $allowed_html) ?></div>
								<?php echo esc_html( $value['suffix'] ); ?> <?php echo ($description); ?>
							</td>
						</tr>
						<?php
					...
					default:
						do_action( 'easy_invoice_admin_field_' . $value['type'], $value );
						break;
				}
			}
		}
		...
		public static function save_fields( $options, $data = null ) {
			...
			foreach ( $options as $option ) {
				 ...
				 * @since 2.4.0
				 */
				$value = apply_filters( 'easy_invoice_admin_settings_sanitize_option', $value, $option, $raw_value );
				 ...
				 * @since 1.0.0
				 */
				$value = apply_filters( "easy_invoice_admin_settings_sanitize_option_$option_name", $value, $option, $raw_value );
				...
				do_action( 'easy_invoice_update_option', $option );
			}
